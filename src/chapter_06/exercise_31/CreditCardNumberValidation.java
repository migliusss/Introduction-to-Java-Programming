...
public class CreditCardNumberValidation {
    ...
    public static int sumOfDoubleEvenPlace(long number) {
        ...
        for (int i = 0; i < strNumber.length(); i += 2) {
            sum += getDigit(Integer.parseInt(strNumber.charAt(i) + "") * 2);
        }
    ...
    /** Return sum of odd-place digits in number. */
    public static int sumOfOddPlace(long number) {
        String strNumber = Long.toString(number); // Convert long to a String.
    ...
    /** Return the number of digits in d. */
    public static int getSize(long d) {
        String strNumber = Long.toString(d); // Convert long to a String.
    ...
    public static long getPrefix(long number, int k) {
        if (getSize(number) > k) {
            String strNumber = Long.toString(number);
            return Long.parseLong(strNumber.substring(0, k));
        }
